...
class SchemaDecompilerTest extends Specification {
    ...
    GraphQLSchema starWarsSchema() {
        def wiring = new RuntimeWiring().forType("Character").typeResolver(resolver).endType().scalar(ASTEROID)
        GraphQLSchema schema = load("starWarsSchema.graphqls", wiring)
        schema
    }
    ...
    GraphQLSchema load(String fileName, RuntimeWiring wiring) {
        ...
        def compile = new SchemaCompiler().compile(new InputStreamReader(stream))
        def schema = new SchemaGenerator().makeExecutableSchema(compile.right().get(), wiring)
        return schema.right().get()
    }
